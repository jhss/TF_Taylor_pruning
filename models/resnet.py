...
def ResNet(
    ...
    **kwargs,
    ...
    if (weights == "imagenet") and (model_name in WEIGHTS_HASHES):
        ...
        base_model = tf.keras.applications.resnet50.ResNet50(include_top, weights, input_tensor, input_shape,
                                                             pooling, classes, 
        ...
        for layer in base_model.layers:
            cur_weights = layer_dict[layer.name].get_weights()
            if len(cur_weights) == 0: continue
            

        for layer in model.layers:
            cur_weights = layer_dict[layer.name].get_weights()
            if len(cur_weights) == 0: continue
            layer.set_weights(lay)
            print("[DEBUG] custom layer: ", layer.name)
        """
...
def block1(x, filters, kernel_size=3, stride=1, conv_shortcut=True, name=None, gate = None):
    """A residual block.
    Args:
      x: input tensor.
    ...
    print("[DEBUG] first gate input shape: ", x.shape)
    x = GateLayer(x.shape[3], x.shape[3])(x)
    ...
    #print("[DEBUG] second gate input shape: ", x.shape)
    x = GateLayer(x.shape[3], x.shape[3])(x)
    ...
    #print("[DEBUG] last gate input shape: ", x.shape)
    if gate is not None:
       x = gate(x)

    return x

def stack1(x, filters, blocks, stride1=2, name=None, gate = None):
    ...
    """
    #print("[DEBUG] First Block")
    x = block1(x, filters, stride=stride1, name=name + "_block1", gate = gate)
    for i in range(2, blocks + 1):
        #print(f"[DEBUG] {i}th Block")
        x = block1(
            x, filters, conv_shortcut=False, name=name + "_block" + str(i), gate = gate
        )
    return x
...
def ResNet50(
    ...
    **kwargs,
    ...
    def stack_fn(x, gate):
        ...
        x = stack1(x, 64, 3, stride1=1, name="conv2", gate = gate_skip64)
        x = stack1(x, 128, 4, name="conv3", gate = gate_skip128)
        x = stack1(x, 256, 6, name="conv4", gate = gate_skip256)
        return stack1(x, 512, 3, name="conv5", gate = gate_skip512)
